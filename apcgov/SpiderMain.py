...
class SpiderMain(object):
    ...
    def pageHandler(self,fileName,driver,page):
        ...
        tablesTag=driver.find_elements_by_xpath('//*[@id="content_ContentPlaceHolder1_SearchResult_GV"]')
        if(len(tablesTag)==0):
            print(str(page)+"Page No Result's Found")
            #结果写入Excel
    ...
    def outPutExcel(self,filename,data,isFirst):
        ...
        wb=None
        addrFile=Path(addr)
        #如果文件存在而且是第一页数据，证明是重试。
        if(addrFile.exists() and isFirst):
            os.remove(addr)
    ...
    def listHandler(self,driver,rootUrl,fileName):
        ...
        # tablesTag=driver.find_elements_by_xpath('//*[@id="content_ContentPlaceHolder1_SearchResult_GV"]')
        # if(len(tablesTag)==0):
        #     print(rootUrl+"，No Result's Found")
        #     return
        #设置语言
        ...
        for common in spider.commonNameList:
            ...
            for i in range(5):
                try:
                    ...
                    print(fileName+"爬取数据结束,retry="+str(i))
                    break
                except BaseException as e:
                    print(fileName+"爬取数据失败,"+str(e))
                    print("重试中,retry="+str(i))
                    continue
    finally:
        driver.quit()
